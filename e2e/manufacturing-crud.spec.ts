...
test.describe("Produksi - Production Order CRUD", () => {
  test("list loads + create form loads", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText("Production", { timeout: 15000 })

    await page.goto("/produksi/production-orders/tambah", { waitUntil: "domcontentloaded" })
    await waitForHydration(page)
    await expect(page.locator("body")).toContainText("Buat", { timeout: 15000 })
  })
})
...
test.describe("Produksi - Work Order CRUD", () => {
  test("list loads + create form loads", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText("Perintah Kerja", { timeout: 15000 })
...
test.describe("Produksi - Products CRUD", () => {
  test("list loads + create form loads", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText("Product", { timeout: 15000 })

    await page.goto("/produksi/products/tambah", { waitUntil: "domcontentloaded" })
    await waitForHydration(page)
    await expect(page.locator("body")).toContainText("Tambah Produk", { timeout: 15000 })
  })
})
