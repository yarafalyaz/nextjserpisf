...
const columns = [
  columnHelper.accessor("documentNo", {
    header: "No. Dokumen",
    cell: (info) => (
      <Link href={`/purchase/requests/${info.row.original.id}`} className="text-primary hover:underline font-medium font-mono">
        {info.getValue()}
      </Link>
    ),
  }),
  ...
  columnHelper.display({
    ...
    cell: (info) => (
      <ActionDropdown
        viewHref={`/purchase/requests/${info.row.original.id}`}
