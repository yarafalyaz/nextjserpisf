            ...
            <DetailTableBody>
              {arRows.map((row, i) => (
                <DetailTableRow key={i}>
                  ...
                  <DetailTableTd align="right">{formatCurrency(row.totalInvoiced)}</DetailTableTd>
                  <DetailTableTd align="right">{formatCurrency(row.totalPaid)}</DetailTableTd>
                  <DetailTableTd align="right" className="font-semibold text-primary">{formatCurrency(row.outstanding)}</DetailTableTd>
                </DetailTableRow>
              ))}
              {arRows.length === 0 && (
              ...
              {arRows.length > 0 && (
                <DetailTableRow className="font-bold border-t-2 border-default">
                  ...
                  <DetailTableTd align="right">{formatCurrency(arRows.reduce((s, r) => s + r.totalInvoiced, 0))}</DetailTableTd>
                  <DetailTableTd align="right">{formatCurrency(arRows.reduce((s, r) => s + r.totalPaid, 0))}</DetailTableTd>
                  <DetailTableTd align="right" className="text-primary">{formatCurrency(totalAR)}</DetailTableTd>
                </DetailTableRow>
              )}
            </DetailTableBody>
          </DetailTable>
            ...
            <DetailTableBody>
              {apRows.map((row, i) => (
                <DetailTableRow key={i}>
                  <DetailTableTd className="font-medium">{row.name}</DetailTableTd>
                  <DetailTableTd align="right">{row.count}</DetailTableTd>
                  <DetailTableTd align="right">{formatCurrency(row.totalBilled)}</DetailTableTd>
                  <DetailTableTd align="right">{formatCurrency(row.totalPaid)}</DetailTableTd>
                  <DetailTableTd align="right" className="font-semibold text-danger">{formatCurrency(row.outstanding)}</DetailTableTd>
                </DetailTableRow>
              ))}
              {apRows.length === 0 && (
              ...
              {apRows.length > 0 && (
                <DetailTableRow className="font-bold border-t-2 border-default">
                  ...
                  <DetailTableTd align="right">{formatCurrency(apRows.reduce((s, r) => s + r.totalBilled, 0))}</DetailTableTd>
                  <DetailTableTd align="right">{formatCurrency(apRows.reduce((s, r) => s + r.totalPaid, 0))}</DetailTableTd>
                  <DetailTableTd align="right" className="text-danger">{formatCurrency(totalAP)}</DetailTableTd>
                </DetailTableRow>
              )}
            </DetailTableBody>
          </DetailTable>
