...
async function getTrialBalanceData(
  ...
  const accounts = Array.from(accountBalances.values()).sort((a, b) =>
...
export default async function FinancialReportsPage({
  ...
  const trialBalance =
    reportType === "trial-balance"
      ? await getTrialBalanceData(params.tanggalMulai, params.tanggalSelesai)
      : null;
  const incomeStatement =
    reportType === "income-statement"
      ? await getIncomeStatementData(params.tanggalMulai, params.tanggalSelesai)
      : null;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Report Selector */}
      <form
        className="bg-surface rounded-xl border border-default shadow-sm p-6 flex gap-4 flex-wrap items-end print:hidden"
      ...
      >
        ...
        <AppDatePicker
          ...
          name="tanggalMulai"
          defaultValue={
            params.tanggalMulai || `${new Date().getFullYear()}-01-01`
          }
          className="w-[180px]"
        />
        <AppDatePicker
          ...
          name="tanggalSelesai"
          defaultValue={
            params.tanggalSelesai || new Date().toISOString().split("T")[0]
          }
          className="w-[180px]"
        />
            ...
            <DetailTable>
                ...
                {trialBalance.accounts.length === 0 ? (
                  ...
                  trialBalance.accounts.map((acc) => (
                    <DetailTableRow key={acc.code}>
                      ...
                      <DetailTableTd>
                        <span className="px-3 py-1 rounded-full text-xs font-medium border border-default bg-background text-muted-foreground cursor-pointer transition-all capitalize hover:border-primary hover:text-primary">
                          {acc.type}
                        </span>
                      </DetailTableTd>
                      ...
                      <DetailTableTd align="right" className="font-bold">
                        {formatCurrency(acc.totalDebit - acc.totalCredit)}
                      </DetailTableTd>
                    </DetailTableRow>
                  ))
                )}
              ...
              <DetailTableFoot>
                <DetailTableFootRow className="font-bold border-t-2 border-default">
                  ...
                  <DetailTableTd align="right">
                    {Math.abs(
                      trialBalance.grandTotalDebit -
                        trialBalance.grandTotalCredit,
                    ) < 0.01
                      ? "SEIMBANG"
        ...
        <div className="grid grid-cols-1 gap-5">
          <div className="bg-surface rounded-xl border border-default shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 px-5 border-b border-default">
              ...
              <span className="text-xl font-bold text-foreground text-success">
          ...
          <div className="bg-surface rounded-xl border border-default shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-4 px-5 border-b border-default">
              ...
              <span className="text-xl font-bold text-foreground text-danger">
          ...
          <div className="bg-surface rounded-xl p-5 px-6 flex items-center justify-center gap-4 shadow-sm border border-default transition-all hover:-translate-y-0.5 hover:shadow-md">
            <div className="flex flex-col items-center">
              ...
              <span
                className={`kpi-value ${incomeStatement.netIncome >= 0 ? "text-success" : "text-danger"}`}
              >
