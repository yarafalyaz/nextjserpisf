...
export default async function TaxGroupsPage({
  ...
  { label: "Tax Groups" },
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Link href="/master/kelompok-pajak/tambah" className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium bg-primary text-white hover:bg-primary-hover hover:-translate-y-px hover:shadow-md transition-all" id="create-tax-group-btn">
          + Tambah Grup Pajak
        </Link>
      </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-3 px-5 border-t border-default">
            ...
            <div className="flex gap-1">
              {page > 1 && (
                <Link href={`/master/kelompok-pajak?halaman=${page - 1}&search=cari=${params.cari || ""}`} className="button button--ghost button--sm">← Sebelumnya</Link>
              )}
              {page < totalPages && (
                <Link href={`/master/kelompok-pajak?halaman=${page + 1}&search=cari=${params.cari || ""}`} className="button button--ghost button--sm">Berikutnya →</Link>
              )}
            </div>
          </div>
        )}
