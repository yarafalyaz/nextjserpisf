  ...
  { label: "Barcodes" },
      ...
      <div className="bg-surface rounded-xl border border-default shadow-sm overflow-hidden">
        <div className="p-3 px-4 flex flex-col gap-3">
          <AppSearchField placeholder="Cari barcode..." action="/master/barcode" />
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-3 px-5 border-t border-default">
            ...
            <div className="flex gap-1">
              {page > 1 && (
                <Link href={`/master/barcode?halaman=${page - 1}&search=cari=${params.cari || ""}`} className="button button--ghost button--sm">← Sebelumnya</Link>
              )}
              {page < totalPages && (
                <Link href={`/master/barcode?halaman=${page + 1}&search=cari=${params.cari || ""}`} className="button button--ghost button--sm">Berikutnya →</Link>
              )}
            </div>
          </div>
        )}
