...
export default async function DeliveryOrderDetailPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      <AppBreadcrumbs items={[{label:"Dasbor",href:"/"},{label:"Penjualan",href:"/sales"},{label:"Surat Jalan",href:"/sales/delivery-orders"},{label:"Detail"}]} />
  ...
  <div className="flex gap-2">
          ...
          <DeleteButton id={deliveryOrder.id} action={deleteDeliveryOrder} />
                  <Link href="/sales/delivery-orders" className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-surface-secondary hover:text-foreground transition-all">← Kembali</Link>
        </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[0.9375rem] text-foreground font-medium">
              <Link href={`/sales/orders/${deliveryOrder.salesOrder.id}`}>{deliveryOrder.salesOrder.documentNo}</Link>
            </span>
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-xs font-medium text-muted uppercase tracking-wide">Customer</span>
            <span className="text-[0.9375rem] text-foreground font-medium">
              <Link href={`/master/customers/${deliveryOrder.salesOrder.customer.id}`}>{deliveryOrder.salesOrder.customer.name}</Link>
            </span>
          </div>
          <div className="flex flex-col gap-1">
