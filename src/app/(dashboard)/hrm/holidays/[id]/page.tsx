...
<div className="flex gap-2">
          <Link href={`/hrm/holidays/${holiday.id}/edit`} className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium bg-primary text-white hover:bg-primary-hover hover:-translate-y-px hover:shadow-md transition-all">Edit</Link>
          <DeleteButton id={holiday.id} action={deleteHoliday} />
                  <Link href="/hrm/holidays" className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:bg-surface-secondary hover:text-foreground transition-all">← Kembali</Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(200px,1fr))] gap-4">
          ...
          {holiday.description && (
            <div className="flex flex-col gap-1" style={{ gridColumn: "1 / -1" }}>
              ...
              <span className="text-[0.9375rem] text-foreground font-medium">{holiday.description}</span>
            </div>
          )}
          <div className="flex flex-col gap-1">
