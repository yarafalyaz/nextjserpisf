  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Link href={`/pengaturan/peran/${role.id}/edit`} className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium bg-primary text-white hover:bg-primary-hover hover:-translate-y-px hover:shadow-md transition-all">
          Edit Role
        </Link>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-default shadow-sm overflow-hidden p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <span className="text-xs font-medium text-muted uppercase tracking-wider">Nama</span>
            <p className="text-sm font-medium text-foreground mt-1 capitalize">{role.name}</p>
          </div>
          <div>
            <span className="text-xs font-medium text-muted uppercase tracking-wider">Guard</span>
            <p className="text-sm font-mono text-foreground mt-1">{role.guardName}</p>
          </div>
          <div>
            <span className="text-xs font-medium text-muted uppercase tracking-wider">Dibuat</span>
      ...
      <div className="bg-surface rounded-xl border border-default shadow-sm overflow-hidden p-6">
        ...
        {role.permissions.length === 0 ? (
          <p className="text-sm text-muted">Belum ada permission yang di-assign</p>
        ) : (
      ...
      <div className="bg-surface rounded-xl border border-default shadow-sm overflow-hidden p-6">
        <h2 className="text-[0.9375rem] font-semibold text-foreground mb-4">
          Users dengan Role ini ({role.users.length})
        </h2>
        {role.users.length === 0 ? (
          <p className="text-sm text-muted">Belum ada user dengan role ini</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {role.users.map((user) => (
