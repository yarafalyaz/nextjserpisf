  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <Link href="/pengaturan/peran/tambah" className="inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg text-sm font-medium bg-primary text-white hover:bg-primary-hover hover:-translate-y-px hover:shadow-md transition-all">
          + Tambah Peran
        </Link>
      </div>
        ...
        <div className="overflow-x-auto">
          <DetailTable>
            ...
            <DetailTableBody>
              {roles.length === 0 ? (
                <DetailTableRow>
                  <DetailTableTd colSpan={6} className="text-center py-10 text-muted-foreground">Belum ada peran</DetailTableTd>
                </DetailTableRow>
              ) : (
                roles.map((role) => (
                  <DetailTableRow key={role.id}>
                    ...
                    <DetailTableTd>{role.users.length} pengguna</DetailTableTd>
