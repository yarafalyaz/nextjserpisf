...
export async function GET(request: NextRequest) {
  ...
  try {
    if (type === "provinces") {
      const csv = readFileSync(join(DATA_DIR, "provinces.csv"), "utf-8")
    ...
    if (type === "regencies" && parentCode) {
      const csv = readFileSync(join(DATA_DIR, "cities.csv"), "utf-8")
      const rows = parseCSV(csv)
    ...
    if (type === "districts" && parentCode) {
      const csv = readFileSync(join(DATA_DIR, "districts.csv"), "utf-8")
      const rows = parseCSV(csv)
      const filtered = rows.filter(r => r[1] === parentCode)
      const data = filtered.map(r => ({ code: r[0], name: toTitleCase(r[2]) }))
      return NextResponse.json(data)
    ...
    if (type === "villages" && parentCode) {
      // Villages are split by province code (first 2 digits)
