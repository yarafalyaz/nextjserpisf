...
import { createPaymentMethod, updatePaymentMethod } from "@/actions/method.actions"
...
export function PaymentMethodForm({ method, generatedCode, enableAutoCode = true }: PaymentMethodFormProps) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        router.push("/master/metode-pembayaran")
        router.refresh()
      } catch (error) {
    ...
    <form onSubmit={onSubmit} className="bg-surface rounded-xl border border-default shadow-sm p-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input id="name" name="name" placeholder="Contoh: Transfer Bank" required defaultValue={method?.name ?? ""} />
        </div>
        <div className="flex flex-col gap-1.5 col-span-full">
          <div className="flex items-center gap-2">
            <Checkbox id="pm-is-active" name="isActive" value="on" defaultChecked={method?.isActive !== false} />
            <Label htmlFor="pm-is-active">Aktif</Label>
          </div>
        </div>
      </div>
