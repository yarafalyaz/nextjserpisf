...
export function TimesheetForm({ employees, projects, tasks = [], timesheet, breakStart, breakEnd }: TimesheetFormProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [employeeId, setEmployeeId] = useState<string | null>(timesheet ? String(timesheet.employeeId) : null)
  const [projectId, setProjectId] = useState<string | null>(timesheet?.projectId ? String(timesheet.projectId) : null)
  const [taskId, setTaskId] = useState<string | null>(timesheet?.taskId ? String(timesheet.taskId) : null)
  const [startTime, setStartTime] = useState(timesheet?.startTime ?? "")
  const [endTime, setEndTime] = useState(timesheet?.endTime ?? "")
  const [hours, setHours] = useState(timesheet?.hours != null ? String(timesheet.hours) : "")
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        ...
        showSuccess(timesheet?.id ? "Data berhasil diperbarui" : "Data berhasil ditambahkan")
      ...
      <FormCard>
        ...
        <FormSection title="Detail Jam">
          <div className="flex flex-col gap-1.5">
            ...
            <AppTimePicker id="startTime" name="startTime" value={startTime} onChange={handleStartChange} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="endTime">Jam Selesai</Label>
            <AppTimePicker id="endTime" name="endTime" value={endTime} onChange={handleEndChange} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="hours">Total Jam *</Label>
            <Input id="hours" name="hours" type="number" step="0.25" min="0.25" max="24" required readOnly value={hours} placeholder="Otomatis dari jam mulai & selesai" className="bg-muted" />
          </div>
        </FormSection>
